...
def create_star_list(filename):
    ...
    with open(filename) as star_file:
        ...
        for line in star_file:
            ...
            star_list.append(convert_star_datatypes(line[:6]))
...
def create_named_star_list(filename):
    ...
    with open(filename) as star_file:
        ...
        for line in star_file:
            ...
            if len(line) == 7:
                data = convert_star_datatypes(line[:6])
                names = line[6].split(";")

                for n in names:
                    n = n.strip()
                    named_stars_dict[n] = data
...
def create_constellations(filename):
    ...
    with open(filename) as file_in:
        file_values = file_in.readlines()

        index = 0
        while index < len(file_values) - 2:
            star_name = file_values[index].strip()
            pairs = int(file_values[index + 1].strip())

            all_star_pairs = []
            for stars_index in range(index + 2, index + pairs + 2):
                star_pair = tuple(file_values[stars_index].strip().split(","))
                all_star_pairs.append(star_pair)

            constellations[star_name] = all_star_pairs
